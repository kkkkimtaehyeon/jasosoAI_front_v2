  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <main className="flex-1 w-full max-w-screen-2xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
        {/* 좌측 사이드바 - AI 가이드 */}
        <aside className="lg:col-span-1 bg-white rounded-lg border border-slate-200 flex flex-col max-h-[calc(100vh-120px)]">
          <div className="flex-1 overflow-y-auto p-6">
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-slate-800 mb-2">작성 가이드</h3>
                <p className="text-sm text-slate-600 leading-relaxed">
                  팀 프로젝트에서 리더십 기술과 경험을 강조하세요. 여러 작업을 관리하고 마감 기한을 맞추는 능력을 강조하세요. 성공적인 프로젝트 결과와 기여에 대한 구체적인 예를 제공하세요.
                </p>
              </div>
              <div>
                  ...
                  {similarResumes.map((resume, index) => (
                    <div key={resume.id} className="transition-all duration-300">
                      ...
                      {expandedCard === resume.id && (
                        <div className="mt-3 p-4 bg-blue-50 border border-blue-200 rounded-lg animate-in slide-in-from-top-2 duration-300">
                          <h4 className="font-semibold text-slate-800 mb-2">Sample Answer:</h4>
        ...
        <section className="lg:col-span-2 bg-white rounded-lg p-6 border border-slate-200">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-200 text-slate-500 hover:bg-slate-300 transition-colors">
                <Plus size={16} />
              </button>
            </div>
          </div>
